# Comprehensive DNS and Connection Fix for Supabase (Fixed Version)
...
if (Test-Path ".env") {
    ...
    if ($databaseLine) {
        ...
        # Extract components - using different variable names to avoid conflicts
...
foreach ($testHostname in $testHosts) {
    Write-Host "   Testing: $testHostname" -ForegroundColor Gray
    try {
        $dnsResult = Resolve-DnsName $testHostname -ErrorAction Stop
        Write-Host "   ✅ $testHostname - RESOLVED" -ForegroundColor Green
        Write-Host "      IP: $($dnsResult.IPAddress)" -ForegroundColor Gray
        $workingHosts += $testHostname
    } catch {
...
if ($workingHosts -contains "db.bervahrnaauhznctodie.supabase.co") {
    ...
    try {
        ...
        if ($directTest) {
            ...
            $connectionTests += @{
                Type = "Direct"
                Hostname = "db.bervahrnaauhznctodie.supabase.co"
                Port = 5432
                Working = $true
            }
        } else {
...
if ($workingConnection) {
    ...
    Write-Host "   Host: $($workingConnection.Hostname)" -ForegroundColor Gray
    ...
    if ($databaseLine -match ":([^@]+)@") {
        ...
        # Create new DATABASE_URL with correct format
        $newDatabaseUrl = "postgresql://postgres.bervahrnaauhznctodie:$currentPassword@$($workingConnection.Hostname):$($workingConnection.Port)/postgres"
        ...
        $newDatabaseUrl = "postgresql://postgres.bervahrnaauhznctodie:$password@$($workingConnection.Hostname):$($workingConnection.Port)/postgres"
    ...
    Write-Host ""
    Write-Host "4. 💻 Run DNS fix:" -ForegroundColor White
    Write-Host "   .\quick-dns-fix-v2.ps1" -ForegroundColor Gray
