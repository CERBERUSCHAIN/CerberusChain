...
CREATE TABLE api_keys (
    ...
    -- Unique constraint to prevent duplicate API keys per service per user
    UNIQUE(user_id, service_name),
    
    -- Constraints
    CONSTRAINT api_keys_service_valid CHECK (service_name IN ('helius', 'discord', 'telegram'))
);
...
-- Password hash for 'CerberusAdmin2024!' using Argon2
INSERT INTO users (username, email, password_hash, is_verified) VALUES 
('admin', 'admin@cerberuschain.com', '$argon2id$v=19$m=19456,t=2,p=1$VGhpcyBpcyBhIHNhbHQ$qLml5cOjCKI0yJOjCKI0yJOjCKI0yJOjCKI0yJOjCKI', true);

-- Create initial bot configuration templates
